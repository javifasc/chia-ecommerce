...
const Cart = () => {
    ...
    // Ahora el carrito sobrevive a una recarga, así que puede apuntar a productos
    // que ya no están en el catálogo: los descartamos en vez de calcular NaN.
    ...
    // Alcanzar el monto no basta: el beneficio solo cubre Rada Tilly.
    const appliesFreeShipping = isFreeShipping && deliveryMethod === 'Envío' && deliveryZone === 'Rada Tilly';
        ...
        return (
            <div className="min-h-screen bg-background-light dark:bg-background-dark flex flex-col items-center justify-center p-6 text-center font-display">
                <div className="w-20 h-20 bg-primary/20 rounded-full flex items-center justify-center mb-6 animate-bounce">
                    <span aria-hidden="true" className="material-symbols-outlined text-4xl text-primary font-bold">check</span>
                </div>
                ...
                <p className="text-slate-500 mb-8 max-w-xs">
                    ...
                    <br />
                    Para agilizar la entrega y coordinar el pago, <span className="font-bold text-slate-700 dark:text-slate-200">confirmá tu pedido por WhatsApp</span>.
                </p>
                ...
                <div className="w-full max-w-xs space-y-3">
                    <a
                        ...
                        className="flex w-full items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-4 rounded-2xl shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all"
                    >
    ...
    return (
        <main className="relative w-full max-w-md lg:max-w-2xl h-[100dvh] overflow-hidden bg-background-light dark:bg-background-dark mx-auto flex flex-col font-display antialiased text-slate-900 dark:text-slate-100 not-italic">
            <header className="flex items-center justify-between p-4 pt-[calc(1rem+env(safe-area-inset-top,0px))] bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md sticky top-0 z-20">
                <button
                    type="button"
                    onClick={() => navigate(-1)}
                    aria-label="Volver"
                    className="flex items-center justify-center w-11 h-11 rounded-full bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                >
                    <span aria-hidden="true" className="material-symbols-outlined text-slate-900 dark:text-white" style={{ fontSize: '20px' }}>arrow_back</span>
                </button>
                    ...
                    {cartItems.length > 0 ? cartItems.map((item) => (
                        <div key={item.productId} className="group relative flex gap-4 p-3 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 hover:border-primary/50 transition-colors text-left animate-in slide-in-from-right duration-300">
                            <div className="relative shrink-0 w-20 h-20 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-700">
                                <img alt="" loading="lazy" decoding="async" className="w-full h-full object-cover" src={item.image} />
                            </div>
                            <div className="flex flex-1 flex-col justify-between py-0.5">
                                ...
                                <div className="flex items-center justify-between mt-2">
                                    <p className="text-[11px] text-green-600 dark:text-primary font-medium bg-green-50 dark:bg-primary/10 px-2 py-0.5 rounded-full">En stock</p>
                                    <div className="flex items-center bg-slate-50 dark:bg-slate-700/50 rounded-full p-1 border border-slate-100 dark:border-slate-600">
                                        ...
                                        >
                                            <span aria-hidden="true" className="material-symbols-outlined text-xs">{item.quantity <= (item.isFractional ? (item.fractionalStep || 0) / 1000 : 1) ? 'delete' : 'remove'}</span>
                                        </button>
                                        <span className="w-14 text-center text-[11px] font-bold text-slate-900 dark:text-white tracking-tighter">{formatWeight(item.quantity, item.isFractional)}</span>
                                        <button
                                        ...
                                        >
                                            <span aria-hidden="true" className="material-symbols-outlined text-xs font-bold">add</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="py-20 text-center flex flex-col items-center">
                            <span aria-hidden="true" className="material-symbols-outlined text-6xl mb-3 text-slate-300 dark:text-slate-600">shopping_basket</span>
                            <p className="text-base font-bold text-slate-700 dark:text-slate-200 mb-1">Tu carrito está vacío</p>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Agregá productos y coordinamos la entrega por WhatsApp.</p>
                            <Link to="/shop" className="px-6 py-3 rounded-full bg-primary text-slate-900 text-sm font-bold shadow-sm active:scale-95 transition-transform">
                                Ver productos
                            </Link>
                        </div>
                    )}
                ...
                {cartItems.length > 0 && (
                    <>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/40 p-1.5 rounded-2xl flex border border-slate-100 dark:border-slate-700/50">
                            ...
                            >
                                <span aria-hidden="true" className="material-symbols-outlined text-lg">local_shipping</span>
                                Envío a Domicilio
                            </button>
                        </div>
                            ...
                            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm animate-in fade-in zoom-in duration-300">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        <span aria-hidden="true" className={`material-symbols-outlined text-xl ${appliesFreeShipping ? 'text-primary' : 'text-slate-400'}`}>
                                            {appliesFreeShipping ? 'check_circle' : 'local_shipping'}
                                        </span>
                                        <span className="text-xs font-bold">
                                            {!isFreeShipping
                                                ...
                                                : appliesFreeShipping
                                                    ? '¡Tenés envío gratis!'
                                                    : 'Envío gratis solo en Rada Tilly'}
                                        </span>
                                    </div>
                                    <span className="text-xs font-bold text-primary">${state.freeShippingThreshold}</span>
                                </div>
                                ...
                                {!isFreeShipping ? (
                                    ...
                                    <p className="text-[11px] text-primary-dark dark:text-primary font-bold">
                                        {appliesFreeShipping
                                            ? 'Alcanzaste el beneficio de envío gratis a domicilio.'
                                            : `Alcanzaste el monto, pero el beneficio aplica solo a Rada Tilly. El envío a ${deliveryZone} tiene costo.`}
                                    </p>
                                )}
                            </div>
                        )}
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                {isAuthenticated && (
                                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-primary/10 rounded-full border border-primary/20">
                                        <span aria-hidden="true" className="material-symbols-outlined text-[12px] text-primary-dark">bolt</span>
                                        <span className="text-[11px] font-bold text-primary-dark uppercase">Datos Guardados</span>
                                    </div>
                                )}
                            </div>
                                ...
                                {deliveryMethod === 'Envío' && (
                                    <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <div className="relative">
                                            <label className="block text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Zona de Envío</label>
                                            <select
                                                value={deliveryZone}
                                            ...
                                            <div className="pointer-events-none absolute right-4 top-[38px] text-slate-400">
                                                <span aria-hidden="true" className="material-symbols-outlined text-sm">keyboard_arrow_down</span>
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Dirección Exacta</label>
                                            <input
                                                type="text"
            ...
            {cartItems.length > 0 && (
                <div className="absolute bottom-0 left-0 w-full bg-background-light dark:bg-background-dark p-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] border-t border-slate-100 dark:border-slate-800 z-30">
                    <button
