...
const OrderManagement = () => {
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark text-text-main dark:text-white font-display h-screen flex flex-col overflow-hidden not-italic">
            <header className="flex items-center justify-between px-5 pt-safe-4 pb-4 bg-background-light dark:bg-background-dark sticky top-0 z-20">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        <span aria-hidden="true" className="material-symbols-outlined text-xl">{viewType === 'kanban' ? 'view_list' : 'view_kanban'}</span>
                    </button>
                    <button
                    ...
                    >
                        <span aria-hidden="true" className="material-symbols-outlined text-xl">mop</span>
                    </button>
                                    ...
                                    {col.orders.length > 0 ? col.orders.map((order) => (
                                        <div key={order.id} className="bg-background-light dark:bg-background-dark p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in duration-300 text-left">
                                            <div className="flex justify-between items-start mb-2">
                                                <span className="text-[11px] font-bold tracking-tighter text-primary-dark dark:text-primary">{order.id}</span>
                                            ...
                                            <div className="space-y-1 my-3 bg-white/50 dark:bg-slate-800/50 p-2 rounded-xl">
                                                {order.items.map((item, i) => (
                                                    <p key={i} className="text-[11px] font-medium text-slate-600 dark:text-slate-400">
                                                        <span className="text-primary-dark dark:text-primary font-bold">{item.quantity}x</span> {item.name}
                                                    </p>
                                                ))}
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                                                ...
                                                {getNextStatus(order.status) && (
                                                    <button
                                                        ...
                                                        className="flex items-center gap-1.5 px-4 py-2 bg-primary text-slate-900 text-[11px] font-bold rounded-xl shadow-md shadow-primary/20 active:scale-95 transition-all"
                                                    >
                                        ...
                                        <div className="py-10 text-center opacity-20">
                                            <span aria-hidden="true" className="material-symbols-outlined text-4xl">inbox</span>
                        ...
                        {groupedOrders.filter(col => col.orders.length > 0).map((col) => (
                            <div key={col.status} className="space-y-3">
                                <div className="flex items-center gap-2 px-1">
                                    ...
                                    <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-400">{col.label} ({col.orders.length})</h2>
                                </div>
                                        ...
                                        <div key={order.id} className="bg-white dark:bg-surface-dark p-4 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 animate-in fade-in slide-in-from-right duration-300">
                                            <div className="flex-1 min-w-0 text-left">
                                                <div className="flex items-center gap-2 mb-0.5">
                                                    <span className="text-[11px] font-bold text-primary-dark dark:text-primary">{order.id}</span>
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span className="text-sm font-bold text-slate-900 dark:text-white whitespace-nowrap">$ {order.total.toFixed(2)}</span>
                                                {getNextStatus(order.status) && (
                                                    <button
                                                        onClick={() => handleStatusChange(order.id, getNextStatus(order.status)!)}
            ...
            <nav className="fixed bottom-0 left-0 right-0 flex gap-2 border-t border-slate-100 dark:border-slate-800 bg-white/95 dark:bg-background-dark/95 backdrop-blur-md px-4 pb-10 pt-2 z-30">
                <Link to="/admin" className="flex flex-1 flex-col items-center justify-end gap-1 text-sage dark:text-slate-400">
                    <span aria-hidden="true" className="material-symbols-outlined text-2xl">bar_chart</span>
                    <p className="text-[11px] font-medium">Panel</p>
                </Link>
                <Link to="/admin/orders" className="flex flex-1 flex-col items-center justify-end gap-1 text-slate-900 dark:text-primary relative">
                    <span aria-hidden="true" className="material-symbols-outlined text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>assignment</span>
                    ...
                    <p className="text-[11px] font-bold">Pedidos</p>
                </Link>
                <Link to="/admin/inventory" className="flex flex-1 flex-col items-center justify-end gap-1 text-sage dark:text-slate-400">
                    ...
                    <p className="text-[11px] font-medium">Inventario</p>
                </Link>
                <Link to="/admin/upload" className="flex flex-1 flex-col items-center justify-end gap-1 text-sage dark:text-slate-400">
                    <span aria-hidden="true" className="material-symbols-outlined text-2xl">add_circle</span>
                    <p className="text-[11px] font-medium">Nuevo</p>
                </Link>
            </nav>
        </div>
    );
